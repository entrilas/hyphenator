...
$settings = new Settings($patternReaderService, $exportService, $config);
$pattern = new Pattern($queryBuilder);
$word = new Word($queryBuilder);
$hyphenationTrie = new HyphenationTrie($settings);

$router = new Router();

// WORDS API ENDPOINTS

$router->post('/api/words', function(array $params = []) use ($word, $hyphenationTrie) {
    $wordController = new WordController($word, $hyphenationTrie);
    print_r($wordController->submit($params));
});

$router->delete('/api/words/:id', function(array $params = []) use ($word, $hyphenationTrie) {
    $wordController = new WordController($word, $hyphenationTrie);
    print_r($wordController->delete($params));
});

$router->update('/api/words/:id', function(array $params = []) use ($word, $hyphenationTrie) {
    $wordController = new WordController($word, $hyphenationTrie);
    print_r($wordController->update($params));
});

$router->get('/api/words', function() use ($word, $hyphenationTrie) {
    $wordController = new WordController($word, $hyphenationTrie);
    print_r($wordController->showAll());
});

$router->get('/api/words/:id', function(array $params = []) use ($word, $hyphenationTrie){
    $wordController = new WordController($word, $hyphenationTrie);
