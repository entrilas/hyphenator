...
class QueryBuilder
{
    ...
    public function getAll(string $table, string $addition = ''): bool
    {
        return $this->action('SELECT *', $table, [], $addition);
    }

    public function getFields(string $table, array $fields, array $where = [], string $addition = ''): bool
    {
        if (is_array($fields)) {
    ...
    public function delete($table, $where = [], $addition = ''): bool
    {
        return $this->action('DELETE', $table, $where, $addition);
    }

    public function action(string $action, string $table, array $where = [], string $addition = ''): bool
    {
        if (empty($where)) {
    ...
    public function join(array $tables, array $fields, array $on)
    {
        ...
        $sql = "SELECT {$this->prepareAliases($fields)} FROM $sql_tables[0] 
                INNER JOIN $sql_tables[1] ON $field1 $operator $field2";
        if(!$this->query($sql)->getError()){
            return $this;
        }
    }
